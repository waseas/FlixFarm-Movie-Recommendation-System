...
export default class MovieSlider extends React.Component {
    ...
    renderMovieCarouselItems(movies) {
        ...
        if (movies.length !== 0) {
            ...
            splicedMovieArray = movieArray.splice(0, Math.floor(movieArray.length / 6) * 6);
            splicedPosterArray = posterArray.splice(0, Math.floor(posterArray.length / 6) * 6);

            let movieSet, posterSet;

            while (splicedMovieArray.length > 0) {
                movieSet = splicedMovieArray.splice(0, 6);
                posterSet = splicedPosterArray.splice(0, 6);
